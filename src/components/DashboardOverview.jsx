...
const ICONS = { Users, FileText, Calendar, Folder, BarChart2 };
...
export default function DashboardOverview({ role, cards }) {
  const items = cards && cards.length > 0
    ? cards.map((c, i) => ({ ...c, icon: Object.keys(ICONS)[i % Object.keys(ICONS).length] }))
    : DEFAULT_CARD_CONFIG[role] || DEFAULT_CARD_CONFIG.admin;
  ...
  return (
    <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {items.map(({ label, value, icon, color }, idx) => {
        ...
        return (
          <div key={label + idx} className="rounded-xl border bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">{label}</p>
                <p className="mt-1 text-2xl font-semibold tracking-tight">{value}</p>
              </div>
              <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${color || 'bg-indigo-50 text-indigo-600'}`}>
                <Icon size={18} />
              </div>
            </div>
          </div>
